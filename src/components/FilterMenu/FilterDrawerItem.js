...
function FilterDrawerItem({ filterItem, currentFilter, filterAction }) {
    ...
    // Instantiate options and values (prevents compile-time error: Cannot read properties of null)
        ...
        <Grid container item className='filter-drawer-item'>
            <Grid container item className='filter-drawer-item__section'>
                <Grid item className='filter-drawer-item__row'>
                    <Typography
                        className='filter-drawer-item__label'
                        variant='body1'
                    >
                        {filterItem.name}:{' '}
                    </Typography>
            ...
            <FormGroup className='filter-drawer-item__options'>
                {options.map((option, index) => (
                    <FormControlLabel
                        ...
                        componentsProps={{
                            typography: {
                                className: 'filter-drawer-item__option-label'
                            }
                        }}
                        control={
                            <Checkbox
                                value={values[index] ?? ''}
                                defaultChecked={defaultCheck.includes(
                                    values[index]
                                )}
